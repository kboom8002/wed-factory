...
export function PricingBandCard({ bands }: { bands: PricingBandItem[] }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-16">
      {bands.map((band) => (
        <div key={band.band_id} className="bg-white rounded-[var(--brand-radius)] border border-gray-200 shadow-sm overflow-hidden flex flex-col transition-all hover:shadow-md hover:border-[var(--brand-primary)]/40 relative">
          
          {/* Header */}
          <div className="p-6 pb-4 border-b border-gray-100 bg-gray-50/50">
            <h3 className="text-xl font-bold text-gray-900 mb-1 tracking-tight">{band.band_label}</h3>
          ...
          {band.major_surcharge_axes && band.major_surcharge_axes.length > 0 && (
            <div className="bg-red-50/80 px-6 py-3 border-b border-red-100 flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {band.major_surcharge_axes.map((axis, i) => (
                  <span key={i} className="bg-white border border-red-200 text-red-700 font-medium text-[11px] px-2 py-0.5 rounded shadow-sm">
                    {axis}
                  </span>
                ))}
              </div>
          ...
          <div className="p-6 flex-1 flex flex-col gap-4">
            {band.included_summary && (
              <div>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-1"><span className="text-green-500 font-normal">✔️</span> 기본 포함 내역</h4>
                <p className="text-gray-700 text-sm leading-relaxed font-medium bg-green-50/30 p-3 rounded-xl">
                  {band.included_summary}
                </p>
              </div>
            )}
            ...
            {band.excluded_summary && (
              <div className="mt-auto pt-4 border-t border-dashed border-gray-100">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-1"><span className="text-red-400 font-normal">❌</span> 불포함 내역 (별도 결제 필요)</h4>
                <p className="text-gray-500 text-sm leading-relaxed p-0">
                  {band.excluded_summary}
                </p>
              </div>
            )}
